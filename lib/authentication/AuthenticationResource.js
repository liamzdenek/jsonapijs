...
function AuthenticationResource(user_finder, password_checker, session_resource, relationship) {
...
AuthenticationResource.prototype.get_by_ids = function(req) {
    ...
    if(auth_header[0] != "Bearer") {
        return Promise.reject(Exception()
            .status(400)
            .title("This API only accepts Authentication: Bearer ....")
        .build());
    }
...
AuthenticationResourceBuilder.prototype.build = function() {
    ...
    return new AuthenticationResource(this.user_finder, this.password_checker, this._session_resource, this._relationship);
}
