...
AuthenticationResourceBuilder.prototype.bcrypt_password_checker = function(field) {
	...
	this.password_checker = function(req, user) {
		...
		var matches = bcrypt.compareSync(req.data.attributes['password'], user.attributes[field]);
		if(matches) {
			return Promise.resolve();
		}
        return Promise.reject(Exception()
            .status(400)
            .title("Couldn't find user OR the password is incorrect")
        	//.meta({debug: "actually password bcrypt -- got: "+req.data.attributes[field]+" -- "+user.attributes[field]})
        .build());
	}
	return this;
}
