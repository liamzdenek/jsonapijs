...
IdByLocalField.prototype.pre_save = function(request, rel_name) {
    return new Promise(function(resolve, reject) {
        if(this.required && (!request.data || !request.data.relationships || !(rel_name in request.data.relationships))) {
            return reject("403 Forbidden -- the "+rel_name+" relationship is not optional");
        }

        if(!(rel_name in request.data.relationships)) {
            return resolve();
        }
