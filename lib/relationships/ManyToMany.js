...
function ManyToMany(config, {src, src_id, src_to_join_name, join_to_dst_name, resource, dst_to_join_name, join_to_src_name, dst, dst_id}) {
	...
	config.push_relationship(src, dst, new Alias(src_to_join_name+"."+join_to_dst_name));
	config.push_relationship(dst, src, new Alias(dst_to_join_name+"."+join_to_src_name));
}
