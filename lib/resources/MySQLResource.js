...
MySQLResource.prototype.startTx = function(req) {
    if(req.arena.mysql_transaction) {
        return req.arena.mysql_transaction;
    }
    req.arena.mysql_transaction = new Promise(function(resolve, reject) {
        this.factory.connection.query("START TRANSACTION", function(err) {
            if(err) { return reject(err); }
            console.log("BEGUN TX");
            resolve("transaction started");
        }.bind(this));
    }.bind(this));
    req.arena.push_after(this.endTx.bind(this,req));
    return req.arena.mysql_transaction;
}

MySQLResource.prototype.endTx = function(req, after) {
    if(!req.arena.mysql_transaction) {
        return;
    }
    console.log("AFTER: ", after);

    return after.then(function(after_data) {
        console.log("ENDING TX ", after_data, req.arena.mysql_transaction);
        return req.arena.mysql_transaction.then(new Promise(function(resolve, reject) {
            this.factory.connection.query("COMMIT", function(err) {
        ...
        .catch(function(outer_err) {
            return new Promise(function(resolve, reject) {
                ...
                this.factory.connection.query("ROLLBACK", function(err) {
                    if(err) { return reject(err); }
                    reject(outer_err);
                }.bind(this))
            }.bind(this))
