...
Config.prototype.push_resource = function(name, resource) {
    if(typeof resource.query != "function") {
        throw "Resource "+name+" is not a resource, does not have .query method -- "+resource.constructor;
    }
...
Config.prototype.get_relationships_by_resource = function(resource_name) {
    ...
    let rels = {};
    if(typeof this.resources[resource_name].get_relationships == "function") {
        let newrels = this.resources[resource_name].get_relationships() || {};
...
Config.prototype.get_relationship_by_resource = function(resource_name, name) {
    if(!resource_name) {
        throw new Error("get_relationship_by_resource requires resource to be defined");
    }
    if(!name) {
        throw new Error("Name must be defined");
    }
    console.log("CHECKING: ", resource_name, name);
    ...
    if(typeof this.resources[resource_name] == "object" && typeof this.resources[resource_name].get_relationships == "function") {
        let rels = this.resources[resource_name].get_relationships() || {};
        if(rels[name]) {
            return rels[name];
        }
    }
